  ...
  const selector = (
    <div className="shared-agent-selector agent-selector">
      ...
      {error && !draft && agents.length > 0 && (
        <button type="button" disabled={loading} onClick={() => requestAgent(agents[0].id)}>
          {t("重试读取助手")}
        </button>
      )}
    </div>
                ...
                {management ? (
                  <>
                    ...
                    <div className="model-page-toolbar">
                      <button type="button" disabled={loading} onClick={() => void refreshModels()}>
                        {t("刷新模型列表")}
                      </button>
                      <span className="hint">{translateNotice(modelStatus)}</span>
                    </div>
                    ...
                    >
                      ...
                      {editorId === "__new__" && draft ? (
                        <fieldset disabled={loading}>
                          <p className="hint">{t("新助手模型暂存为草稿，创建时一并保存。")}</p>
                          <ChatModelFields draft={draft} models={models} patch={patchDraft} />
                          <button type="button" onClick={openOverview}>
                            {t("返回助手管理继续创建")}
                          </button>
                        </fieldset>
                  ...
                  <>
                    ...
                    <nav className="workspace-anchors" aria-label={t("知识库分区跳转")}>
                  ...
                  meta && (
                    <>
                      ...
                      {isEditablePage(routeId) ? (
                        ...
                        <p className="hint">{t("状态：暂未开放")}</p>
                      ) : (
                        <button type="button" disabled={loading} onClick={() => openOverview()}>
                          {t("打开原配置")}
                        </button>
                      )}
                    </>
