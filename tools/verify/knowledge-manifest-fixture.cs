...
internal static class KnowledgeManifestFixture
{
    ...
    public static int Main()
    {
        ...
        for (int version = 1; version <= current; version++)
        {
            ...
            if (v < current) Reject(delegate { Manifest.Parse(Json(v)); });
        ...
        Reject(delegate { Manifest.Parse(Json(current + 1), true); });
        Reject(delegate { Manifest.Parse(Json(0), true); });
        Reject(delegate { Manifest.Parse(Json(current).Replace("\"businessSchemaVersion\":" + current, "\"businessSchemaVersion\":\"" + current + "\"")); });
        Reject(delegate { Manifest.Parse(Json(current).Replace("\"product\":\"superstring\"", "\"product\":\"other\"")); });
